	...
	IN	INT		txPowerTuning)
{
	...
	for (i = 0; i < 2 * NUM_EEPROM_TX_G_PARMS; i++)
	{
		if ((ChannelTxPower[i] > 36) || (ChannelTxPower[i] < -6))
			...
			pAd->TxPower[i].Power = ChannelTxPower[i] + txPowerTuning;
 
		DBGPRINT(RT_DEBUG_INFO, "Tx power for G channel %d : 0x%02x\n", pAd->TxPower[i].Channel, (UCHAR)(pAd->TxPower[i].Power));
	}
	...
	for (i = 0; i < MAX_NUM_OF_A_CHANNELS; i++)
	{
		...
		DBGPRINT(RT_DEBUG_INFO, "Tx power for A channel %d : 0x%02x\n", pAd->TxPower[i + 14].Channel, (UCHAR)(pAd->TxPower[i + 14].Power));
	}
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	pAd->BbpTuning.R17LowerBoundG		  = 0x00; // for best RX sensibility
	pAd->BbpTuning.R17UpperBoundG		  = 0x00; // for best RX noise isolation to prevent false CCA
