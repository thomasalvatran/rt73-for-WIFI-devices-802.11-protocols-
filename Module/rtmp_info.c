...
{
	PRTMP_ADAPTER pAdapter = (PRTMP_ADAPTER) get_netdev_priv(dev);
	int 	chan = -1;
...
INT rt73_ioctl(
	...
	IN	INT					cmd)
{
	RTMP_ADAPTER						*pAd = get_netdev_priv(net_dev);
	struct iwreq						*wrq = (struct iwreq *) rq;
...
#if 1
		case RTPRIV_IOCTL_TXPOWER:
			RTMPIoctlTxPower(pAd, wrq);
			break;
...
//STUPID: Cannot yet set txPower on-the-fly while card runs :(

	memset(msg, 0, 1024);

	if (wrq->u.data.length > 1)
	{
		if (strchr(wrq->u.data.pointer, 'x') == NULL)
		{
			sscanf(wrq->u.data.pointer, "%d=%d", &parseChan, &parsePower);
		} else {
			sscanf(wrq->u.data.pointer, "%d=0x%02x", &parseChan, (UINT *) &parsePower);
			if (parsePower > 128) parsePower -= 256;  //Fixing unparsability of signed hex values
		}

		if ((parseChan < 0) || (parseChan > 14)) {
			sprintf(msg+strlen(msg), "\n\nChannel is out of range or unparseable");
			wrq->u.data.length = strlen(msg);
			Status = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length);
			return;
		}
		if ((parsePower < -6) || (parsePower > 36)) {
			sprintf(msg+strlen(msg), "\n\nPower is out of range or unparseable");
			wrq->u.data.length = strlen(msg);
			Status = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length);
			return;
		}

		sprintf(msg+strlen(msg), "\n\nSetting Channel %d to txpower 0x%02x (%d)", parseChan, (UCHAR) parsePower, (INT) parsePower);

		for (i = 0; i < 2 * NUM_EEPROM_TX_G_PARMS; i++)
		{
			if ((pAd->TxPower[i].Channel == parseChan) || (parseChan == 0))
				pAd->TxPower[i].Power = parsePower;
		}
	...
	for (i = 0; i < 2 * NUM_EEPROM_TX_G_PARMS; i++)
		sprintf(msg+strlen(msg), "    %3d | 0x%02x (%2d)\n", pAd->TxPower[i].Channel, (UCHAR)(pAd->TxPower[i].Power), pAd->TxPower[i].Power);
/*
	sprintf(msg+strlen(msg), "\nSetting Tx Power:\niwpriv %s txpower <chan>=<power>", pAd->net_dev->name);
	sprintf(msg+strlen(msg), "\nSetting all Channels to same power setting:\niwpriv %s txpower 0=<power>", pAd->net_dev->name);
	sprintf(msg+strlen(msg), "\nValid Values for Transmit Power: -6 to 36 (0xFA to 0x24).\nWARNING: Setting values higher than they are after loading the driver\nis NOT recommended and MAY DAMAGE YOUR HARDWARE! - USE AT OWN RISK!");
*/
	sprintf(msg+strlen(msg), "\nHow to set Tx Power:");
	sprintf(msg+strlen(msg), "\n1. Shutdown the interface          : ifconfig %s down", pAd->net_dev->name);
