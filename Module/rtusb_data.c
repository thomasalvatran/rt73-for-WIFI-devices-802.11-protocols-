...
INT RTMPSendPackets(
	...
	IN	struct net_device	*net_dev)
{
	PRTMP_ADAPTER	pAd = get_netdev_priv(net_dev);
	NDIS_STATUS 	Status = NDIS_STATUS_SUCCESS;
	//INT 			Index;
...
//	PCIPHER_KEY		pWpaKey = NULL;
	...
	if (pAd->PortCfg.BssType == BSS_MONITOR && pAd->bAcceptRFMONTx == TRUE)
	{
		...
		if (pSkb->data[1] & 4) {
			//Packet fragment, do not increase Sequence Number
			bIsFragment = TRUE;
		} else {
			//Last fragment, increase Sequence
			bIsFragment = FALSE;
		}
		...
		RTUSBWriteTxDescriptor(pAd, pTxD, CIPHER_NONE /*CipherAlg*/, 0, 0xff /*KeyIdx*/,
					FALSE /*bAckRequired*/, bIsFragment, FALSE, 4 /*RetryMode*/,
					IFS_BACKOFF /*FrameGap*/, pAd->PortCfg.TxRate /*TxRate*/,
