...
static INT txPowerTuning = 0;

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
MODULE_PARM(debug, "i");
MODULE_PARM(ifname, "s");
MODULE_PARM(firmName, "s");
MODULE_PARM(txPowerTuning, "i");
#else
module_param(debug, int, 0);
module_param(ifname, charp, 0444);
module_param(firmName, charp, S_IRUGO );
module_param(txPowerTuning, int, 0);
#endif
...
MODULE_DESCRIPTION("Ralink RT73 802.11abg WLAN Driver - k2wrlz modifications 3.0.3");
...
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) get_netdev_priv(net_dev);
...
/*
 * Since COMPAT_NET_DEV_OPS has been removed from the kernel, 
 * we're kinda stuck with using the new net_device_ops API.
 * This change occured at 2.6.31.  This supports the new interface
 */
...
static inline int common_probe(PRTMP_ADAPTER pAd)
{
	...
	NICReadEEPROMParameters(pAd, txPowerTuning);
				...
				const struct usb_device_id *id_table)
{
	...
	/* However, this works fine for ASPj on 2.6.27, 2.6.28 and 2.6.29.3 with 32 and 64bits ;) */
...
out:
	free_netdev(netdev);
out_nonetdev:
	//kfree(pAd);	//Not needed, because of using alloc_etherdev() now to create space for pAd. So free_netdev does this for us already.
out_noalloc:
	usb_put_dev(dev);
					  ...
					  const struct usb_device_id *id)
{
	...
	/* Removed because no more CONFIG_COMPAT_NET_DEV_OPS (03-Nov-2009) */
	/*
	netdev->open = usb_rtusb_open;
	netdev->stop = usb_rtusb_close;
	netdev->hard_start_xmit = RTMPSendPackets;
	...
	*/
	...
	{// find available
    	...
    	if (ifname == NULL)
			strcpy(pAd->net_dev->name, "ralan%d");
    	else
...
INT __init usb_rtusb_init(void)
{
    ...
    printk("rt73 k2wrlz modification 3.0.3 (-----------------------------July 17, 2012\n");
	#ifdef DBG
    RTDebugLevel = debug;
	#else
